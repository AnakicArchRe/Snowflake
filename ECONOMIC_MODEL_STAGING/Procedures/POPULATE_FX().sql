CREATE OR REPLACE PROCEDURE ECONOMIC_MODEL_STAGING.POPULATE_FX()
RETURNS NUMBER(38,0)
LANGUAGE SQL
AS
begin
        ...
        with 
            ...
            dates as (
                ...
                where date <= 
                    greatest
                    (
                        (select max(fxdate) from economic_model_raw.submission), 
                        (select max(boundfxdate) from economic_model_raw.layer)
                    )
            )
            -- get fx records
            , fxRows as (
